...
public class PolyvPointRewardEffectWidget extends FrameLayout {
    ...
    public PolyvPointRewardEffectWidget(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PolyvPointRewardEffectWidget(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        LayoutInflater.from(context).inflate(R.layout.plv_widget_point_reward_effect, this, true);
        init();
    }
    // </editor-fold>

    // <editor-fold defaultstate="collapsed" desc="设置积分打赏事件生产者">
    public void setEventProducer(IPolyvPointRewardEventProducer producer) {
        pointRewardEventProducer = producer;
        prepareEventProducer();
    }

    private void prepareEventProducer() {
        pointRewardEventProducer.prepare(new IPolyvPointRewardEventProducer.OnPreparedListener() {
            @Override
    ...
    private void fetchEventForTopItem() {
        pointRewardEventProducer.fetchEvent(new IPolyvPointRewardEventProducer.IPLVOnFetchRewardEventListener() {
            @Override
    ...
    private void makeOnceAnim(final RelativeLayout itemView, final View zoomView, final Runnable animEnd) {
        ...
        enterAnim.setAnimationListener(new Animation.AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                //停留两秒
                PolyvRxTimer.delay(2 * 1000, new Consumer<Object>() {
                    @Override
                ...
                magnify.setAnimationListener(new Animation.AnimationListener() {
                    ...
                    @Override
                    public void onAnimationEnd(Animation animation) {
                        PolyvRxTimer.delay((long) (0.05 * 1000), new Consumer<Object>() {
                            @Override
                            public void accept(Object o) throws Exception {
    ...
    private void loadImage(String url, ImageView iv) {
        ...
        PolyvImageLoader.getInstance().loadImage(iv.getContext(), url, iv);
    }
